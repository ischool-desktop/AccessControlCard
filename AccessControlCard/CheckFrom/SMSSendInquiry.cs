...
namespace AccessControlCard
{
    public partial class SMSSendInquiry : BaseForm
    {
        ...
        private void btn_Select_Click(object sender, EventArgs e)
        {
            ...
            sb.Append("select student.id,student_number,class.class_name,seat_no,name,$access_control_card.student_cardno.card_no,");
            sb.Append("$access_control_card.history.use_time,$access_control_card.history.oclock_name,");
            sb.Append("$access_control_card.history.use_type ");

            //來自student
            //join class
            sb.Append("from student left outer join class on class.id=student.ref_class_id ");
            //join access_control_card.student_cardno
            sb.Append("inner join $access_control_card.student_cardno on $access_control_card.student_cardno.ref_student_id=student.id ");
            //join access_control_card.history
            sb.Append("left outer join $access_control_card.history ");
            sb.Append("on $access_control_card.history.card_no=$access_control_card.student_cardno.card_no ");

            //卡號不得為null
            sb.Append("where $access_control_card.student_cardno is not null ");

            //卡號 / 學號 / 姓名
            if (!string.IsNullOrEmpty(textBoxX1.Text.Trim()))
            {
                sb.Append(string.Format("and ($access_control_card.student_cardno.card_no like '%{0}%' ", textBoxX1.Text.Trim()));
            ...
            //日期判斷式
            sb.Append(string.Format("and ($access_control_card.history.use_time>='{0}' ", dateTimeInput1.Value.ToShortDateString()));
            sb.Append(string.Format("and $access_control_card.history.use_time<='{0}')", dateTimeInput2.Value.AddDays(1).ToShortDateString()));

            //上學 / 放學
            if (checkBoxX1.Checked && checkBoxX2.Checked)
                sb.Append("and ($access_control_card.history.use_type='01' or $access_control_card.history.use_type='02')");
            else if (checkBoxX1.Checked)
                sb.Append("and $access_control_card.history.use_type='01'");
            else if (checkBoxX2.Checked)
                sb.Append("and $access_control_card.history.use_type='02'");
            else
            {
