...
namespace AccessControlCard
{
    public partial class frmQueryNoAccessControl : FISCA.Presentation.Controls.BaseForm
    {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            strBuilder.Append("select student.id,student_number,class.class_name,seat_no,name,$access_control_card.student_cardno.card_no from student ");
            strBuilder.Append(" left outer join class on class.id=student.ref_class_id ");
            strBuilder.Append(" inner join $access_control_card.student_cardno on $access_control_card.student_cardno.ref_student_id=student.id where $access_control_card.student_cardno is not null and student.ref_class_id is not null and student.status in (1,2)");
            ...
            strHistory.AppendLine(" select card_no,use_type from $access_control_card.history where to_char(use_time,'YYYY/MM/DD')='" + dateTimeInput1.Value.ToString("yyyy/MM/dd")+"'");
