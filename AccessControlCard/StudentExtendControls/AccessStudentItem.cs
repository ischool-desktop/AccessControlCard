...
namespace AccessControlCard
{
    //社團的學生增減
    //必須有另一個背景模式進行處理
    [FISCA.Permission.FeatureCode("AccessControlCard.StudentExtendControls.AccessStudent.cs", "門禁刷卡歷程")]
    public partial class AccessStudentItem : DetailContentBase
    {
        ...
        void Save_BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(CarNo))
            {
                ...
                {
                    #region 只有一人使用時
                    if (cardList[0].RefStudentID == int.Parse(_Record.ID)) //是否為本學生
                    {
                        //新增一筆卡號
                        InsertNewCard(CarNo);
                    }
                    else //錯誤
                    {
                        ...
                        StudentRecord sr = Student.SelectByID(cardList[0].RefStudentID.ToString());
                ...
                else if (cardList.Count > 1)
                {
                    ...
                    {
                        StudentRecord sr = Student.SelectByID(each.RefStudentID.ToString());
                        StudentList.Add(sr);
                        sb.AppendLine(SetStudent(sr));
                    }
                    sb.AppendLine("\n您是否要將重覆資料刪除後新增?");
        ...
        private void InsertNewCard(string CarNo)
        {
            ...
            acc.CardNo = CarNo;
            acc.RefStudentID = int.Parse(_Record.ID);
            list.Add(acc);
        ...
        void BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (_Record != null)
            {
                ...
                if (accList.Count == 1)
                {
                    ...
                    //取得學生刷卡歷程
                    StringBuilder sb = new StringBuilder();
                    sb.Append("select $access_control_card.history.oclock_name,$access_control_card.history.use_type,$access_control_card.history.use_time from $access_control_card.history ");
                    sb.Append(string.Format("where $access_control_card.history.card_no='{0}'", _AccessControlCard.CardNo));
                ...
                else if (accList.Count > 1)
                {
                    ...
                    StringBuilder sb = new StringBuilder();
                    sb.Append("select $access_control_card.history.oclock_name,$access_control_card.history.use_type,$access_control_card.history.use_time from $access_control_card.history ");
                    sb.Append("where $access_control_card.history.card_no='" + _AccessControlCard.CardNo + "'");
