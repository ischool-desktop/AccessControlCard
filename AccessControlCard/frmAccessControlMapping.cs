    ...
    public partial class frmAccessControlMapping : FISCA.Presentation.Controls.BaseForm
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (StudentCards.Count > 0)
            {
                try
                {
                    ...
                    {
                        AccessControlCard Card = Cards.Find(x => ("" + x.RefStudentID).Equals(vStudentCard.StudentID));

                        //檢查是否有相同卡號，但是系統編號不一樣的學生
                        AccessControlCard DupCard = DupCards.Find(x => x.CardNo.Equals(vStudentCard.CardNo) &&
                            !("" + x.RefStudentID).Equals(vStudentCard.StudentID));
                        ...
                        if (Card == null)
                        {
                            ...
                            Card.RefStudentID = int.Parse(vStudentCard.StudentID);
                    ...
                    foreach (StudentCard vStudentCard in StudentCards)
                    {
                        Commands.Add("UPDATE student SET sms_phone='" + vStudentCard.StudentPhone + "' WHERE id =" + vStudentCard.StudentID);
                    }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(txtName.Text))
            {
                #region 當使用者輸入條件時
                StringBuilder strBuilder = new StringBuilder();
                strBuilder.Append("select student.id,student_number,class.class_name,class.grade_year,seat_no,name, sms_phone,$access_control_card.student_cardno.card_no from student ");
                strBuilder.Append(" left outer join class on class.id=student.ref_class_id ");
                strBuilder.Append(" left outer join $access_control_card.student_cardno on $access_control_card.student_cardno.ref_student_id=student.id ");
                strBuilder.Append(" where student.status=1 and student.name like '%" + txtName.Text + "%' or student.student_number like '%" + txtName.Text + "%' or $access_control_card.student_cardno.card_no like '%" + txtName.Text + "%'");
        ...
        private void SavePreviousData(String id)
        {
            ...
            String ids = "('" + id + "')";
            StringBuilder strBuilder = new StringBuilder();
            strBuilder.Append("select id,name,sms_phone,card_no from student ");
            strBuilder.Append("left join $access_control_card.student_cardno on $access_control_card.student_cardno.ref_student_id=student.id ");
